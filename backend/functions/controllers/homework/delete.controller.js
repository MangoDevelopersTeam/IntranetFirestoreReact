...
controllers.deleteFeedback = async (req, res) => {
    let db = admin.firestore();
    ...
    if (idSubjectParam == null || idUnitParam == null || idFileParam == null || idStudentParam == null || idFeedbackParam == null) 
    {
        code = "PARAMS_NULL";
        message = "Asegurate de que hayas completado los campos del formulario";
        type = "error";
    ...
    await db.collection("courses").doc(idSubject).collection("units").doc(idUnit).collection("files").doc(idFile).collection("answers").doc(idStudent).collection("feedback").doc(idFeedback).delete()
    .then(() => {
        code = "PROCESS_OK";
    ...
    .catch(error => {
        code = error.code;
        message = "Ha ocurrido un error al momento de eliminar el feedback";
        type = "error";
        status = 500;
    })
    .finally(() => {
